...
防止模型文件膨胀和重复条目。本 skill 曾在早期犯过"错误保留旧条目导致两套并行"的错，
以下规则从那次错误中提炼。
...
1. **目标文件是否已有同类条目？** — 搜索 models/ 中所有 JSON 文件的同名 section/device
2. **已有条目的样本来源是什么？** — 查看 papers_analyzed 列表
3. **样本量和置信度？** — 根据 papers_count 和 confidence 字段判断
4. **本次是"合并"还是"新建"？** — 相似度 ≥ 0.7 → 合并；< 0.7 → 新建
5. **是否有冲突？** — 新模式与既有模式的方向不一致时标记"待验证"
...
```
合并时：
1. 更新 rhetorical_moves 的频率（取平均值）
2. 更新 typical_length_ratio（取平均值）
3. 追加新的 move（如不重复）
4. 更新 papers_analyzed 列表
5. 重新计算置信度
6. 记录合并日志
```
...
```
新建时：
1. 创建新的 section/device 条目
2. 设置初始频率 (frequency = 1 / papers_count)
3. 标记来源论文
4. 记录创建日志
```
...
| 论文数 | 置信度级别 | 可靠性 | 可用场景 |
|--------|----------|--------|---------|
| 1 篇 | 低 (< 0.3) | 仅供参考 | 不能用于写作指导 |
| 2–4 篇 | 中 (0.3–0.6) | 有参考价值 | 可作为辅助建议 |
| 5+ 篇 | 高 (> 0.6) | 可靠性好 | 可作为主要写作依据 |

## 模型文件整洁度检查

每次模型更新后运行：

1. **同类型条目检查**：同一 rhetorical_function 只保留 1 条（如有多个则合并）
2. **空条目清理**：无内容的 section/device 删除
3. **过期标记清理**：超过 90 天未更新的"待验证"标记提示复查
4. **papers_analyzed 去重**：同一 paper_id 只保留 1 次
...
标记为"待验证"
    ↓
记录矛盾详情（哪个维度、什么矛盾、来源论文）
    ↓
报告主人："模型 {model_id} 发现矛盾：[描述]，已标记待验证"
```
